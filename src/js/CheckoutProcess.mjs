...
export default class CheckoutProcess {
  ...
  calculateItemSummary() {
    ...
    this.itemTotal = this.list.reduce(
      (total, item) => total + parseFloat(item.FinalPrice || 0),
      0,
    );
  ...
  displayItemSummary() {
    ...
    const orderItemsElement = document.querySelector(
      `${this.outputSelector} #order-items`,
    );
  ...
  async checkout(form) {
    ...
    try {
      ...
      const result = await services.checkout(formData);
  ...
  handleSuccessfulCheckout() {
    console.log("Handling successful checkout...");
